...
const readline = require('readline'); // Ditambahkan untuk input nomor
const pino = require('pino'); // Logger untuk membersihkan terminal
...
async function startSkyMe() {
    ...
    const sock = makeWASocket({
        version,
        logger: pino({ level: 'silent' }), // Log JSON dimatikan agar bersih
        auth: {
            creds: state.creds,
    ...
    if (!sock.authState.creds.registered) {
        ...
        console.log("==========================================\n");
        const phoneNumber = await question('Masukkan nomor WhatsApp (contoh: 62895xxx): ');
    ...
    sock.ev.on('messages.upsert', async (chatUpdate) => {
        try {
            ...
            if (userId.includes('@g.us')) {
				...
				const groupName = groupMetadata.subject; // Ini adalah Nama Grupnya
				// 2. Cek apakah Nama Grup mengandung kata kunci
...
async function prosesChatDanHistory(sock, m, userId, body, historyPath) {
    ...
    try {
        ...
        if (lyricMatch) {
            ...
            // Menghilangkan balasan AI agar hanya lirik yang terkirim
            cleanReply = "";
        }
